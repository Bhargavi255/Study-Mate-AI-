...
class LLM:
    ...
    def load_local(self):
        ...
        assert self.local_model is not None
        self.tokenizer = AutoTokenizer.from_pretrained(self.local_model)
        # load model (device_map="auto" if using accelerate & transformers that support it)
        self.model = AutoModelForCausalLM.from_pretrained(self.local_model, device_map="auto")
        self._loaded = True
    ...
    def generate(self, prompt: str, max_tokens: int = 256) -> str:
        if self.mode == "hf_api":
            ...
            if not self.hf_model:
                return "ERROR: hf_model not specified for HF Inference API mode."
            if not (self.hf_token):
                return "ERROR: HUGGINGFACE_TOKEN not provided."
            api_url = f"https://api-inference.huggingface.co/models/{self.hf_model}"
            headers = {"Authorization": f"Bearer {self.hf_token}"}
            ...
            try:
                ...
                return f"Inference API request failed: {e}"
            if resp.status_code != 200:
                return f"Inference API error {resp.status_code}: {resp.text}"
            try:
                out = resp.json()
...
def ingest_pdfs(files, chunk_size=800, overlap=200):
    """
    Extract, chunk, and build vector store.
    `files` is a list of Gradio File-like dicts or local file paths.
    Returns summary info dict.
    """
    all_texts = []
    metadatas = []

    if not files:
        return {"n_chunks": 0, "n_files": 0}

    file_count = 0
    for f in files:
        # handle Gradio's uploaded-file representation (dict) or direct path
        path = None
        filename = None
        if isinstance(f, dict):
            # gradio's file object often has "name" (original filename) and "tmp_path" or "tmp_file"
            filename = f.get("name") or f.get("orig_name")
            path = f.get("tmp_path") or f.get("name") or f.get("file") or f.get("tmpfile")
        elif hasattr(f, "name") and os.path.exists(getattr(f, "name")):
            # file-like object, use its path
            path = f.name
        ...
        for i, c in enumerate(chunks):
            all_texts.append(c)
            metadatas.append({"source": filename or os.path.basename(path), "chunk_id": i, "text": c})
    ...
    STATE["chunks"] = all_texts
    STATE["metadatas"] = metadatas
    return {"n_chunks": len(all_texts), "n_files": file_count}
...
def answer_question(question: str, k: int = 4, llm_mode: str = "hf_api", hf_model: str = None, hf_token: str = None, local_model: str = None) -> Tuple[str, List[dict]]:
    ...
    prompt = (
        "You are StudyMate, an AI academic assistant. Use the provided context from the user's uploaded PDFs to answer the question. "
        "If the answer is not contained in the context, say you don't know and provide suggestions to locate the answer."
        f"\n\nContext:\n{context}\n\nQuestion: {question}\n\nAnswer:"
    )

    # initialize LLM wrapper if necessary
    # ensure hf_token preference: parameter -> env var -> previously set
    if hf_token:
        os.environ["HUGGINGFACE_TOKEN"] = hf_token

    if STATE.get("llm") is None or STATE["llm"].mode != llm_mode:
        STATE["llm"] = LLM(mode=llm_mode, hf_model=hf_model, hf_token=(hf_token or os.environ.get("HUGGINGFACE_TOKEN")), local_model=local_model)

    # update hf_model/token if LLM exists but model changed
    llm = STATE["llm"]
    llm.hf_model = hf_model or llm.hf_model
    llm.hf_token = hf_token or llm.hf_token or os.environ.get("HUGGINGFACE_TOKEN")
    llm.local_model = local_model or llm.local_model
...
with gr.Blocks(css=css, title="StudyMate — Hackathon-ready", theme=gr.themes.Soft()) as app:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=0.7):
            ...
            k_slider = gr.Slider(label="# retrieved chunks (k)", minimum=1, maximum=10, step=1, value=4)
            download_btn = gr.Button("Download Transcript")
            download_file = gr.File(label="Download JSON Transcript", visible=False)
    ...
    def on_ingest(files, chunk_size, overlap, hf_token_field):
        ...
        # files may be list of dicts (gradio) or file-like objects
        info = ingest_pdfs(files, chunk_size=int(chunk_size), overlap=int(overlap))
        msg = f"Indexed {info['n_chunks']} chunks from {info['n_files']} files. Ready to ask questions."
        if hf_token_field:
            os.environ["HUGGINGFACE_TOKEN"] = hf_token_field
        return msg

    ingest_btn.click(on_ingest, inputs=[upload, chunk_size_slider, overlap_slider, hf_token_input], outputs=[ingest_output])

    def on_ask(question, k, llm_mode, hf_model, hf_token, local_model, chat_history, transcript):
        if not question or question.strip() == "":
            return chat_history, transcript, gr.update()
        # call answer pipeline
        answer, sources = answer_question(question, k=int(k), llm_mode=llm_mode, hf_model=hf_model, hf_token=hf_token, local_model=local_model)
        # append to chat
        chat_history = (chat_history or []) + [("You", question), ("StudyMate", answer)]
        # update transcript
        transcript = (transcript or []) + [{"q": question, "a": answer, "sources": sources}]
        # update sources box text
        sources_md = "### Top Sources\n"
        for s in sources:
            snippet = (s.get("snippet") or "")[:200].replace("\n", " ")
            sources_md += f"- **{s.get('source','unknown')}** (dist={s.get('dist',0):.3f}) — {snippet}...\n"
        return chat_history, transcript, sources_md

    ask_btn.click(on_ask,
                  inputs=[user_input, k_slider, llm_mode_radio, hf_model_input, hf_token_input, local_model_input, chatbot, transcript_store],
                  outputs=[chatbot, transcript_store, sources_box])
    ...
    def download_transcript(transcript):
        ...
        with open(fname, "w", encoding="utf-8") as f:
            json.dump(transcript, f, ensure_ascii=False, indent=2)
        # return a file object path to the File component
        return fname

    download_btn.click(download_transcript, inputs=[transcript_store], outputs=[download_file])

    # Run
    demo = app

if __name__ == "__main__":
    # optionally set host/port via env vars
    host = os.environ.get("STUDYMATE_HOST", "0.0.0.0")
    port = int(os.environ.get("STUDYMATE_PORT", 7860))
